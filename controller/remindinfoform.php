...
class remindinfoform extends basis
{
	function index(){
	...
	function add(){
		...
		$this -> display('content/Clientfb/remind.html');
	}
	...
	function addremind(){
        ...
        if(!empty($result_user['uid']) && !empty($data['content'])){
			...
			$user = spClass('remindinfoform');
			$newid = $user->create($newrow);
			$this->success("回复【".$data['replyid'].'】成功',spUrl("ucenter","Userremind"));
        }
 ...
 //    }
	...
	function showinfo(){
        ...
        $remind = spClass('remindinfo');
        $result_remind = $remind->find($conditions);
        if($_SESSION["quser"]['uid'] == $data['replyid'])
        {
	      $remind->update($conditions,array('mark'=>1)); 
        }
        
		$user = spClass('lib_user'); 
	    $conditions = array( 'uid' => $result_remind['uid'] );
	    $result_user = $user->find($conditions);  // 分页
	    $result_remind['uid'] = $result_user;

	    $user = spClass('lib_user'); 
	    $conditions = array( 'uid' => $result_remind['replyid'] );
	    $result_user = $user->find($conditions);  // 分页
	    $result_remind['replyid'] = $result_user;

		echo json_encode($result_remind);
	}
