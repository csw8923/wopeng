...
class Images{
	...
	function move_uploaded(){        //移动上传文件
		...
		if($this->isRename){
			...
			//$file_newname = date("YmdHis").rand(1000,9999)."00.".$extension; //重命名新文件， 00表示为上传的为
			$file_newname = date("YmdHis").rand(1000,9999)."00.".$extension; //重命名新文件， 00表示为上传的为原图原图
		}else{
       ...
       $info  = $this->getImageInfo($image);
         if($info !== false) {
            ...
            if ($suofang==1) {
				...
				$thumbname01 = substr_replace($image,"thumb_01.".$type,$length);		//大头像
				$thumbname02 = substr_replace($image,"thumb_02.".$type,$length);		//小头像
				$imageFun($thumbImg,$thumbname01,100);
				$imageFun($thumbImg,$thumbname02,100);

                $thumbImg01 = imagecreatetruecolor(250,250);
                imagecopyresampled($thumbImg01,$thumbImg,0,0,$_POST['x'],$_POST['y'],250,250,$_POST['w'],$_POST['h']);

				$thumbImg02 = imagecreatetruecolor(68,68);
                imagecopyresampled($thumbImg02,$thumbImg,0,0,$_POST['x'],$_POST['y'],68,68,$_POST['w'],$_POST['h']);
