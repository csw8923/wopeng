<?php
$conditions_youji = array('types'=>$this->info,'linkid' => $this->id,'ispass' => 0);
	$s_youji = spClass('youjiinfo'); 
	$this->youji = $s_youji->findAll($conditions_youji,'id DESC',NULL,"4");
	//dump($this->youji);
$conditions_answers = array('types'=>$this->info,'linkid' => $this->id,'ispass' => 0);
	$s_answers = spClass('answersinfo'); 
	$this->answers = $s_answers->findAll($conditions_answers);
	//dump($this->answers);

$conditions_locals = array('types'=>$this->info,'linkid' => $this->id,'ispass' => 0);
	$s_locals = spClass('localsinfo'); 
	$this->locals = $s_locals->findAll($conditions_locals); 

$conditions_visits = array( 'types' => $this->info,'linkid' => $this->id,'ispass' => 0);
	$s_visits = spClass('visitsinfo'); 
	$this->visits = $s_visits->findAll($conditions_visits); 

$conditions_comment = array( 'types' => $this->info,'linkid' => $this->id,'ispass' => 0);
