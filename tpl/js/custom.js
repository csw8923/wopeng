...
function $$(tag,id){return (id==undefined?document:this.getId(id)).getElementsByTagName(tag);}
...
var atuokey = '';
function init(){//初始化
  getId('btn_bg').innerHTML=getId('btn').innerHTML;
  getId('focus_change').removeChild($$('div','focus_change')[0]);
  poptit('ts',0);
  opa('pics',0);
    classNormal();
    $$('li','btn')[0].className = 'current';
  getId('focus_change').onmouseover = function() {
        atuokey = true;
    clearInterval(auto);
    }
    getId('focus_change').onmouseout = function() {
        atuokey = false;
