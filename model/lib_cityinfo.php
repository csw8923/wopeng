<?php
class lib_cityinfo extends spModel
{
    var $pk = "id"; // 数据表的主键
    var $table = "cityinfo"; // 数据表的名称
    ...
    public function getCatalogList($id=0,$level=0)
    {
        ...
        foreach ($results as $cat){
            $id = $cat['id'];
			      $pid = $cat['pid'];
            $name = $str.$cat['citystate'];
			      $seq = $cat['seq'];
			      $ismenu = $cat['ismenu'];
			      $part = $cat['part'];
			      $another = $cat['another'];
			      $servicephone = $cat['servicephone'];
            $value = array('id'=>$id,'pid'=>$pid,'citystate'=>$name,'area'=>$area,'seq'=>$seq,'ismenu'=>$ismenu,'part'=>$part,'another'=>$another,'servicephone'=>$servicephone);
            $catlist[] = $value;
	...
	public function getCatalogSub($id=0,$level=0)
    {
        $conditions = array('pid'=>$id);
        $results = $this->findAll($conditions,'seq ASC');
        $catlist = array();
