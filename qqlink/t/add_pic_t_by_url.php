...
<?php
    if(isset($_POST)&&!empty($_POST)){
        echo "<table>";
		preg_match("/.*\.(.*?)$/i",$_POST["pic"],$aTemp);
		$sFile = date("YmdHis").".".$aTemp[1];
		if(download($_POST["pic"],$sFile)){
...
?>
<form action="add_pic_t_by_url.php" method="POST">
<table>
  <tr>
    <td class="narrow-label">微博图片</td>
    <td><input name="pic" class="input" />(请输入图片网址，请确保存在)</td>
  </tr>
  <tr>
