		...
		if($sContent!==FALSE){
			...
			if($sContent!==FALSE){
				...
				if(intval($aConfig["debug"])==1){
					echo "<tr><td class='narrow-label'></td><td><input type='button' onclick='location.href=\"../index.php\";' class='button' value='点此返回首页' /></td></tr>";
						...
						if($sContent!==FALSE){
							...
							if($aResult["ret"]==0){
								...
								$qqname = $aResult["data"]["name"];
								$email = $aResult["data"]["email"];
								$_SESSION["qqname"] = $aResult["data"]["name"];
								$nick = $aResult["data"]["nick"];
								$_SESSION["nick"] = $aResult["data"]["nick"];
					...
					//echo $nickname;
					echo "<script>location.href='../../index.php?c=login&a=qqregdata&type=qq&openid=".$_SESSION["openid"]."&email=".$email."&nick=".$nickname."&name=".$qqname."';</script>";
				}
			}
		}
    //}
