...
<body>
<h1>QQ互联 - 获取登录用户的收听列表</h1>
<div class="list-div">
<?php
    echo "<table>";
    $sUrl = "https://graph.qq.com/relation/get_idollist";
    $aGETParam = array(
    ...
    if($sContent!==FALSE){
        ...
        if($aResult["ret"]==0){
            ...
            if(intval($aResult["data"]["hasnext"])===0){
                echo "<tr><td class='narrow-label'></td><td><input type='button' value='下一页' onclick='location.href=\"get_idollist.php?p=".(isset($_GET["p"])&&is_numeric($_GET["p"])?(intval($_GET["p"])+1):"1")."\";' class='button' /></td></tr>";
            }
        }else{
